...
/*
Después de introducir el numero de casos de prueba que queremos considerar, se calcula el diametro total del arbol (primero se calcula la distancia de nodo derecgo a nodo izquierdo
de la raiz y sucesivamente sus hijos y se devuelve el mayor) y la distancia (usando cola auxiliar) entre nodos
*/
...
private:
	...
	*/
	...
	//el caso peor es que los nodos estén lo más separados posibles (altura del arbol), distancia a la cual llamaremos n. Por lo tanto el coste esta en O(n)
	bool contNodos_rec(const T& e, const Link& raiz, int& numNodos, queue<T>& colaAux) const {
		if (raiz == nullptr) {
			return false;
		}
		else if (cmp(e, raiz->elem)) {
			if (colaAux.empty() && raiz->izq != nullptr) { //si la cola esta vacia, pusheamos el izquierdo y sumamos 1
				numNodos++;
		...
		else if (cmp(raiz->elem, e)) { //lo mismo pero por la derecha
	...
	int diametro_rec(Link n) const {
		if (n != nullptr) {
			int leftHeight = height(n->izq); //calculamos la altura del hijo izquierdo O(n)
			int rightHeight = height(n->der); //calculamos la altura del hijo derecho O(n)
...
public:
	...
	int distancia(const T& a, const T& b){
		...
		Link root = raiz;
		queue<T> colaAux;
		contNodos_rec(a, root, numNodosA, colaAux); //O(n), n altura del árbol
		contNodos_rec(b, root, numNodosB, colaAux);  //O(n), n altura del árbol
...
void calcula(queue<int>& c, bst<int>& arbol) {
		int d = arbol.diametro();
		if (d < 0) {
			cout << 0 << endl; //caso del arbol vacio
		}
		else {
			cout << d << endl;
		}
